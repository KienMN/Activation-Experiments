...
parser.add_argument(
  '--activation', '-a',
  type=str,
  choices=['relu', 'prelu', 'dprelu', 'frelu'],
...
parser.add_argument(
  ...
  type=str2bool,
  default=False,
  help='Whether to apply batch normalization or not'
)
...
# Save results
metrics.to_csv(work_dir + '/metrics/' + model_name + '_metrics.csv', index=False)
model.save(work_dir + '/weights/' + model_name + '_final_epoch.h5')